...
You can build your own `EventStorageAdapter` class by following the [`EventStorageAdapter` interface](https://github.com/castore-dev/castore/blob/main/packages/core/src/storageAdapter.ts):
...
> You can also take a look at the [`InMemoryStorageAdapter` implementation](https://github.com/castore-dev/castore/blob/main/packages/inmemory-event-storage-adapter/src/adapter.ts) which is quite simple.
...
> This ensures that executed [`Commands`](../../the-basics/#command) are not subject to [race conditions](https://en.wikipedia.org/wiki/Race_condition) and are accordingly retried.
